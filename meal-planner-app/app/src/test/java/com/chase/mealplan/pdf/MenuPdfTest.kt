...
class MenuPdfTest {
    ...
    fun writesMenuWithRecipePages() {
        ...
        val planned = listOf(
            ...
            PlannedMeal(PlanEntryEntity(3, "2026-09-28", Slot.BREAKFAST, 2), oats),
        ) + List(12) { PlannedMeal(PlanEntryEntity(10L + it, "2026-09-29", Slot.SNACK, 2), oats) }

        val out = File(context.cacheDir, "menus/test.pdf")
        MenuPdf(photos).write(week, planned, includeRecipes = true, includePhotos = true, out = out)
        val bytes = out.readBytes()
        assertTrue(bytes.size > 1000)
        assertTrue(String(bytes, 0, 5) == "%PDF-")
        // Menu page + tacos (long recipe spills onto a second page) + oats.
        val pages = Regex("/Type\\s*/Page[^s]").findAll(String(bytes, Charsets.ISO_8859_1)).count()
        assertTrue("expected at least 4 pages, got $pages", pages >= 4)
    }
}

    ...
    DropdownMenu(expanded = open, onDismissRequest = { open = false }) {
        if (app.lifeBoard.isInstalled()) {
            DropdownMenuItem(
                ...
                onClick = { open = false; app.lifeBoard.open(null) },
            )
        }
        DropdownMenuItem(
            text = { Text("Back up to file…") },
            onClick = { open = false; exportLauncher.launch("planboard-backup-${LocalDate.now()}.json") },
        )
        DropdownMenuItem(
            text = { Text("Restore from backup…") },
            onClick = { open = false; importLauncher.launch(arrayOf("application/json", "application/octet-stream", "*/*")) },
        )
    }
